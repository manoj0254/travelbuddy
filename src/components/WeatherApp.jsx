  ...
  function WeatherApp() {
    ...
    const api = "http://api.openweathermap.org/data/2.5/weather?q=hyderabad,in&appid=d411261204dffd5d3adca18bbca3f8ba";
    ...
    useEffect(() => {
      fetch(api)
        ...
        .then(data => {
          ...
          setWeather(data);    // save response in state
        })
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 text-left mb-10">
              <div className='text-left'>
                <p> <span className='text-4xl font-bold'>{weather.name}</span>, <span>{weather.sys.country}</span></p>
                <p>Humidity: {weather.main.humidity}</p>
                <h2>Clouds: {weather.clouds.all}%</h2>
                <p className='text-3xl'>{Math.round(weather.main.temp - 273.15)}°C</p>
              </div>
              <div>
                  <img 
...
/>
        ...
        drop-shadow-xl drop-shadow-xl mt-2'>
              <div className="grid grid-cols-3 gap-4 text-left mb-3">
                ...
                <div><p>Visibility: {weather.visibility / 1000} Km</p></div>
              </div>
